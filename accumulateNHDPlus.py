...
Returns: sumDict - dictionary where the catchment ID is the key and the data is a list of catchment statistics and
                   accumulated statistics
"""
def aggregateRas(shapes, stat, rasPath, convFact, rasType, rasVals):
    print("Calculating...")
    sumDict = {}
...
"""
Method: accumulate()
Purpose: accumulate the catchment statistics
Params: rasType - 0 or 1 specifying if raster is continuous or classed
        ...
        convFact - the conversion factor specified by the user to be applied to accumulated values
Returns: sumDict - dictionary where the catchment ID is the key and the data is a list of catchment statistics and
                   accumulated statistics
"""
def accumulate(rasType, rasVals, allUp, sumDict, convFact):
...
Params: sumDict - dictionary containing catchment statistics
        ...
        numClasses - integer denoting the number of statistics to sum (# of classes for classed data or tabular data
...
"""
rasType = 2 
statType = "SUM" # -- only required for continuous data
outputType = [1, 1]
convFact = 1 / 1000000 
VPUzone = "02" 
catchID = 'COMID'
tabID = 'COMID'
tabCols = ['CRP', 'FORE', 'INR', 'IR', 'MO', 'PAS', 'TCI', 'TCT', 'TG', 'WAT', 'WLF', 'WLO', 'WLT']
...
if __name__ == "__main__":
    ...
    OUT_DIR = os.path.join(ABS_DIR, "Output")

    #File Paths
    NHD_SHP = os.path.join(IN_DIR, "NHDv2_CHWAv2.shp") 
    sassPath = os.path.join(ABS_DIR, "Output", "SassDB_02.csv") #nhdplusv2_us.sas7bdat
    inputData = os.path.join(IN_DIR, "NHDv2_CHWAv2.shp")#"IMP_P6LU.tif") #input raster or tabular catchment data
    ...
    if rasType == 0 or rasType == 1: #Raster data - continuous or categorical
        ...
        if cellSize == 0:
            ...
            if len(shapes) == 0:
                ...
                rasVals, sumDict = aggregateRas(shapes, statType, inputData, convFact, rasType, rasVals)
                if len(sumDict) == 0:
                    invalidData = True
                else:
    ...
    if not invalidData:
        if outputType[0] == 1:
            writeTable(sumDict, OUT_DIR, "CBW_HRLU_082920.csv", rasVals)
        if outputType[1] == 1:
            writeShapefile(sumDict, NHD_SHP, OUT_DIR, "CBW_HRLU_082920.shp", rasVals)
